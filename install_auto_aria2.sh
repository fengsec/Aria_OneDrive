...
download_aria2_conf(){
     mkdir "${file}" && cd "${file}"
	 wget --no-check-certificate -N "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/aria2.conf"
	 [[ ! -s "aria2.conf" ]] && echo -e "${Error} Aria2 配置文件下载失败 !" && rm -rf "${file}" && exit 1
	 wget --no-check-certificate -N "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/Aria2/dht.dat"
	 [[ ! -s "dht.dat" ]] && echo -e "${Error} Aria2 DHT文件下载失败 !" && rm -rf "${file}" && exit 1
	 echo '' > aria2.session
	 stty erase '^H' && read -p "请输入aria2密钥(默认naraku):" pass
	 sed -i 's/^rpc-secret=naraku/rpc-secret='${pass}'/g' ${aria2_conf}
...
service_aria2(){
     if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
install_autoUpload(){
	 ...
	 mkdir -p ${auto_upload}
     cd ${auto_upload}
	 wget --no-check-certificate -q -O json-parser "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/Business/json-parser"
	 wget --no-check-certificate -q -O onedrive "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/Business/onedrive"
	 wget --no-check-certificate -q -O onedrive-d "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/Business/onedrive-d"
	 wget --no-check-certificate -q -O onedrive-authorize "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/onedrive-authorize"
	 wget --no-check-certificate -q -O onedrive-base "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/Business/onedrive-base"
	 wget --no-check-certificate -q -O onedrive.cfg "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/onedrive.cfg"
	 chmod -R a+x ${auto_upload}
	 ln -sf ${auto_upload}/onedrive /usr/local/bin/
	 ln -sf ${auto_upload}/onedrive-d /usr/local/bin/
     rm -rf $(basename "$0")
	 wget -P ${file}  --no-check-certificate -N "https://raw.githubusercontent.com/fengsec/Aria_OneDrive/master/shell/autoUpload.sh"
...
#   ${Green_font} 站在巨人肩膀上(chaoxi && Eleven)二次开发 ${Font}
#   ${Green_font} 作者：Eleven ${Font}
#   ${Green_font} 网站：https://www.naraku.cn ${Font}
# ====================================================
